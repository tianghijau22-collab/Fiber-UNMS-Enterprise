...
echo ">>> 2. Configuring Laravel (.env)..."
cd ${APP_DIR}
if [ ! -f .env ]; then
...
echo ">>> 6. Configuring Nginx for PHP 8.4..."
sudo tee /etc/nginx/sites-available/fiber-unms > /dev/null << 'EOF_NGINX'
server {
    listen 80;
    listen [::]:80;
...
echo ">>> 7. Configuring Supervisor Queue Worker..."
sudo tee /etc/supervisor/conf.d/fiber-unms-worker.conf > /dev/null << 'EOF_SUP'
...
echo ">>> 8. Configuring Laravel Scheduler Cron..."
...
echo "=========================================================="
echo "    DEPLOYMENT COMPLETE! ACCESS: http://103.89.6.125      "
echo "=========================================================="
