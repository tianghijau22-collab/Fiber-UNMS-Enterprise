...
export default function OltManagement() {
  ...
  }, [oltData?.onu_list, selectedPortFilter]);
                  ...
                  <div className="flex items-center space-x-2">
                    {selectedPortFilter && (
                      <button
                        ...
                        className="px-3 py-1.5 rounded-xl bg-emerald-100 dark:bg-emerald-950/60 hover:bg-emerald-200 dark:hover:bg-emerald-900/80 text-emerald-800 dark:text-emerald-300 font-bold text-xs transition-colors flex items-center space-x-1.5 border border-emerald-300 dark:border-emerald-800 disabled:opacity-50"
                        title="Query SNMP langsung hanya untuk port ini"
                      >
                        {loadingPortOnus ? <Spinner /> : <span>⚡ Refresh Port Ini</span>}
                      </button>
                    )}
                    {selectedPortFilter && (
                      <button
