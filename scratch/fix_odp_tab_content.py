...
# 1. Remove `{selectedOdp && (...)}` modal from the end of `NetworkInfrastructure()`
modal_pattern = r'\{\/\* Modal Detail Port & Monitoring Sinyal ODP \*\/\}\s*\{selectedOdp && \([\s\S]*?\)\}\s*(?=\s*<\/div>\s*\);\s*\})'
...
if target in content:
    content = content.replace(target, odp_modal_jsx + '\n' + target)
